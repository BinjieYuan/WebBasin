...
export default {
  ...
  mounted(){
    this.mapElbs=this.$store.getters.storeMap;
    //   this.initMap();
    this.initController();
    //   this.initLayers();
    this.listenDraw();
  ...
  watch:{
    basinShow(newVal, oldVal){
      this.basinShowTemp=newVal
    },
    immediate: true
  },
  data(){
  ...
  methods:{
    ...
    //   ]),
      ...
      initController(){
        ...
        this.drawingLayerGroup.addTo(this.mapElbs);
      },
      listenDraw(){
          console.log('listenDraw Begin');
          var _this = this;
          this.mapElbs.on('pm:create', e=>{
              _this.showDownloadGeoJSONBtn = false;
              _this.drawingLayerGroup.clearLayers();
              _this.drawingLayerGroup.addLayer(e.layer);
            //   _this.$store.commit("SET_MAP_OBJECT", this.mapElbs);
          });
          this.mapElbs.on('pm:remove', e=>{
              _this.showDownloadGeoJSONBtn = false;
              _this.drawingLayerGroup.clearLayers();
            //   _this.$store.commit("SET_MAP_OBJECT", this.mapElbs);
          });
          this.mapElbs.on('click', e=>{
      ...
      showRiverLayer(show){
          if(show){
              ...
              this.riverLayer = L.tileLayer('http://172.21.212.233:8085/tile/{z}/{x}/{y}.png').addTo(this.mapElbs);
          }
          else{
      ...
      showBasinsLayer(show){
          if(show){
              ...
              try{
                ...
                //   this.$store.commit("SET_MAP_OBJECT", this.mapElbs);
              }catch{}
          }
      },
      changeBainsType(){
      ...
      showWholeBasinLayer(show){
          if(show){
              this.wholeBasinLayer = L.tileLayer('http://172.21.213.155:8085/tile/{z}/{x}/{y}.png').addTo(this.mapElbs);
          }
          else{
      ...
      showSimulationURL(wmsUrl, startTime, endTime){
          ...
          this.mapElbs.timeDimension.setAvailableTimes(newAvailableTimes, 'replace');
      ...
      addUpstreamClickPoint(lon, lat){
          ...
          this.mapElbs.addLayer(this.ucpLayer);
        //   this.$store.commit("SET_MAP_OBJECT", this.mapElbs);
      },
      runLFModel(name){
      ...
      addGeoJSONToMap(GeoJSONStr, color){
          ...
          //平移至数据位置
          this.mapElbs.fitBounds(geoJsonLayer.getBounds());
      },
